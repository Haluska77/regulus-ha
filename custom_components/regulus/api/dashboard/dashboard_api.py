    ...
    def generate_response(self, schema_xml_map: Dict[str, str], registry_mapper: Dict[str, str]) -> DashboardResponseSchema:
        ...
        return DashboardResponseSchema(
            outdoorTemperature = sensor("Outdoor Temperature", 
                                        get_value_from_map(schema_xml_map, "outdoorTemperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:sun-thermometer-outline", Platform.SENSOR),
            rcTariff = sensor("RC Tariff", 
                                          get_value_from_map(schema_xml_map, "rcTariff", registry_mapper, converter=lambda v: v == "1"),
                                          "", "none", "mdi:cash-clock", Platform.BINARY_SENSOR),
            holiday = sensor("Holiday", 
                                          get_value_from_map(schema_xml_map, "holiday", registry_mapper, converter=lambda v: v == "1"),
                                          "", "none", "mdi:beach", Platform.BINARY_SENSOR),
            heatPumpRunningStatus= sensor("Heat Pump Running Status", 
                                          get_value_from_map(schema_xml_map, "heatPumpRunningStatus", registry_mapper, converter=lambda v: v != "0"),
            ...
            heatPumpOutletTemperature = sensor("Heat Pump Outlet Temperature", 
                                        get_value_from_map(schema_xml_map, "heatPumpOutletTemperature", registry_mapper), 
            ...
            heatPumpInletTemperature = sensor("Heat Pump Inlet Temperature", 
                                        get_value_from_map(schema_xml_map, "heatPumpInletTemperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:thermometer", Platform.SENSOR),
            zone1Status = sensor("Zone 1 Running Status", 
                                          get_value_from_map(schema_xml_map, "zone1RunningStatus", registry_mapper, converter=lambda v: v != "0"),
            ...
            zone1ActualTemperature = sensor("Zone 1 Actual Temperature", 
                                        get_value_from_map(schema_xml_map, "zone1Temperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:thermometer", Platform.SENSOR),
            zone1RequiredTemperature = sensor("Zone 1 Required Temperature", 
                                        get_value_from_map(schema_xml_map, "zone1RequiredTemperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:thermometer", Platform.SENSOR),
            zone1ActualHeatingWaterTemperature = sensor("Zone 1 Heating Water Temperature", 
                                        get_value_from_map(schema_xml_map, "zone1HeatingWaterTemperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:thermometer", Platform.SENSOR),
            zone1RequiredHeatingWaterTemperature = sensor("Zone 1 Required Heating Water Temperature", 
                                        get_value_from_map(schema_xml_map, "zone1RequiredHeatingWaterTemperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:thermometer", Platform.SENSOR),
            zone2Status = sensor("Zone 2 Running Status", 
                                          get_value_from_map(schema_xml_map, "zone2RunningStatus", registry_mapper, converter=lambda v: v != "0"),
            ...
            zone2ActualTemperature = sensor("Zone 2 Actual Temperature", 
                                        get_value_from_map(schema_xml_map, "zone2Temperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:thermometer", Platform.SENSOR),
            zone2RequiredTemperature = sensor("Zone 2 Required Temperature", 
                                        get_value_from_map(schema_xml_map, "zone2RequiredTemperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:thermometer", Platform.SENSOR),
            zone2ActualHeatingWaterTemperature = sensor("Zone 2 Heating Water Temperature", 
                                        get_value_from_map(schema_xml_map, "zone2HeatingWaterTemperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:thermometer", Platform.SENSOR),
            zone2RequiredHeatingWaterTemperature = sensor("Zone 2 Required Heating Water Temperature", 
                                        get_value_from_map(schema_xml_map, "zone2RequiredHeatingWaterTemperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:thermometer", Platform.SENSOR),
            akuStatus = sensor("Aku Running Status", 
                                          get_value_from_map(schema_xml_map, "akuRunningStatusFromHeatPump", registry_mapper, converter=lambda v: v == "1"),
                                          "", BinarySensorDeviceClass.RUNNING, "mdi:water-boiler", Platform.BINARY_SENSOR),
            akuTopTemperature = sensor("Aku Top Temperature", 
                                        get_value_from_map(schema_xml_map, "akuTopTemperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:thermometer", Platform.SENSOR),
            akuBottomTemperature = sensor("Aku Bottom Temperature", 
                                        get_value_from_map(schema_xml_map, "akuBottomTemperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:thermometer", Platform.SENSOR),
            akuRequiredTemperature = sensor("Aku Required Temperature", 
                                        get_value_from_map(schema_xml_map, "akuRequiredTemperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:thermometer", Platform.SENSOR),
            waterStatus = sensor("Water Running Status", 
                                          get_value_from_map(schema_xml_map, "waterRunningStatusFromHeatPump", registry_mapper, converter=lambda v: v == "1"),
            ...
            waterActualTemperature = sensor("Water Actual Temperature", 
                                        get_value_from_map(schema_xml_map, "waterSwitchingSensorTemperature", registry_mapper), 
            ...
            waterRequiredTemperature = sensor("Water Required Temperature", 
                                        get_value_from_map(schema_xml_map, "waterRequiredTemperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:water-thermometer-outline", Platform.SENSOR),
            solarStatus = sensor("Solar Running Status", 
                                        get_value_from_map(schema_xml_map, "solarRunningStatus", registry_mapper, converter=lambda v: v == "1"), 
            ...
            solarPanelTemperature = sensor("Solar Panel Temperature", 
                                        get_value_from_map(schema_xml_map, "solarPanelTemperature", registry_mapper), 
                                      "°C", SensorDeviceClass.TEMPERATURE, "mdi:thermometer", Platform.SENSOR),
            circulationStatus = sensor("Circulation Running Status", 
                                        get_value_from_map(schema_xml_map, "circulationRunningStatus", registry_mapper, converter=lambda v: v == "1"), 
